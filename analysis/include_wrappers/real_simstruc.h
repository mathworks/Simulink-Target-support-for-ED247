 ...
 * 
 * Get the current size of dimension dIdx of the signal at output port pIdx and record this port index.
 * @param    S             SimStruct representing an S-Function block.
 * @param    pIdx          Output port index being polled.
 * @param    dIdx          Index of dimension being polled.
 * @return   An int_T value indicating the size of dimension dIdx
...
#define ssGetCurrentOutputPortDimensionsAndRecordIndex(S, pIdx, dIdx)  \
       *((_ssSafelyCallGenericFcnStart(S)((S),GEN_FCN_ADD_OUTPUT_VARDIMS_PORT_INDEX, (pIdx), NULL) \
         _ssSafelyCallGenericFcnEnd) ? &(ssGetCurrentOutputPortDimensions(S, pIdx, dIdx)) \
...
# define ssCallSystemWithTid(S,element,tid)   \
    (((S)->callSys.fcns[(element)] != NULL) ? \
     ((*(S)->callSys.fcns[(element)])         \
      ...
      ((ssGetErrorStatus(S) == NULL) && (ssGetLocalErrorStatus(S) == NULL)) : (0)) : (1))
 ...
 * @param    S           SimStruct representing an S-function block.
 * @param    set         Define how to treat an S-function simulation state when saving and restoring the model operating point. 
 */
#define ssSetOperatingPointCompliance(S, set)   \
    (S)->sizes.flags.simStateCompliance = (set)

/**
 * @brief ssSetOperatingPointVisibility
 * 
 * Specify whether to make the S-function's simulation state visible in the operating point of the model.
 * @param    S           SimStruct representing an S-Function block.
 ...
 */
#define ssSetOperatingPointVisibility(S, vis)   \
...
* @return A pointer (char_T *) to a character vector that identifies the last error message.
*/
#define ssGetLocalErrorStatus(S) \
          S->blkInfo.blkInfo2->localErrorStatus.str                               /* (const char_T*) */
