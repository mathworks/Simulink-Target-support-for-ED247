...
RUN sudo mkdir -p $ED247_LOC && sudo chown -R matlab:users $ED247_LOC && cp -r /tmp/export/ed247/* $ED247_LOC
RUN sudo mkdir -p $LIBXML_LOC && sudo chown -R matlab:users $LIBXML_LOC && cp -r /tmp/export/libxml2/* $LIBXML_LOC
